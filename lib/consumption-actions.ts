...
export async function getConsumables(includeArchived = false) {
    const supabase = await createSupabaseServerActionClient()
...
export async function getArchivedConsumables() {
    const supabase = await createSupabaseServerActionClient()
    ...
    try {
        ...
        if (error) {
            console.error("Database error:", error)
            return { data: [], error: "Failed to fetch archived consumables" }
        }

        return { data: data as Consumable[], error: null }
    } catch (error) {
        console.error("Get archived consumables error:", error)
        return { data: [], error: "An unexpected error occurred" }
    }
}
...
export async function archiveConsumable(prevState: any, formData: FormData) {
    const supabase = await createSupabaseServerActionClient();
    ...
    try {
        ...
        if (!existingConsumable) {
            return { error: "Consumable not found or you don't have permission to archive it" };
        }

        // Archive consumable by setting archived = true
        const { error: updateError } = await supabase
            .from("consumables")
            .update({
                archived: true,
        ...
        if (updateError) {
            console.error("Archive consumable error:", updateError);
            return { error: "Failed to archive consumable" };
        }

        revalidatePath("/");
        return { success: "Consumable archived successfully" };
    } catch (error) {
        console.error("Archive consumable error:", error);
        return { error: "An unexpected error occurred" };
    }
}

export async function unarchiveConsumable(prevState: any, formData: FormData) {
    const supabase = await createSupabaseServerActionClient();
    ...
    try {
        ...
        if (!existingConsumable) {
            return { error: "Consumable not found or you don't have permission to unarchive it" };
        }

        // Unarchive consumable by setting archived = false
        const { error: updateError } = await supabase
            .from("consumables")
            .update({
                archived: false,
        ...
        if (updateError) {
            console.error("Unarchive consumable error:", updateError);
            return { error: "Failed to unarchive consumable" };
        }

        revalidatePath("/");
        return { success: "Consumable unarchived successfully" };
    } catch (error) {
        console.error("Unarchive consumable error:", error);
        return { error: "An unexpected error occurred" };
    }
}
